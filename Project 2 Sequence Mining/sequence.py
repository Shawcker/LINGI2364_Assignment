...
def PrefixSpan(filepath1, filepath2, k):
    ...
    Root = Node('root', [cursor, new_transactions])
    MinFrequency = 0.6
    MinSupport = MinFrequency * len(transactions)
    valid_list = []
    for item in items:
        Depth_First(item, items, Root, MinSupport, valid_list)
    ...
    while number < k:
        ...
        for pair in total_list:
            for item in pair[1]:
                Depth_First(item, items, pair[0], MinSupport, valid_list)
        ### Check current supports ###
        all_sequence = All_Frequent_Sequence(valid_list)
        all_support = Get_Support(all_sequence)
        different_support = set(all_support)
...
def Depth_First(item, items, parent, MinSupport, valid_list):
    """Given an item, find the position"""
    number = 0
    parent_cursor = parent.dataset[0]
    dataset = parent.dataset[1]
    new_dataset = []
    new_cursor = []
    # print('item', item)
    for index in range(len(dataset)):
        current_transaction = dataset[index]
        # print('origin', current_transaction)
        ...
        if item in transaction_items:
            ...
            if candidates[-1] >= start_index:
                current_transaction = copy.deepcopy(dataset[index])
                for order in range(len(candidates)):
                    if candidates[order] >= start_index:
                        new_cursor.append(candidates[order])
                        current_transaction[transaction_items.index(item)][1] = candidates[order :]
                        # print('change', current_transaction)
                        new_dataset.append(current_transaction)
                        number += 1
                        break
    # print(new_cursor)
    # print(new_dataset)
    if number >= MinSupport:
        new_node = Node([item, number], [new_cursor, new_dataset], parent)
        valid_list.append(new_node)
        for item in items:
            Depth_First(item, items, new_node, MinSupport, valid_list)
